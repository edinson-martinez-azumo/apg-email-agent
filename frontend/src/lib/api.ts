...
export const emailsApi = {
  ...
  analyze: (id: string) =>
    http.post(`/api/v1/emails/${id}/analyze`).then(r => r.data),
  reAnalyze: (id: string) =>
    http.post(`/api/v1/emails/${id}/re-analyze`).then(r => r.data),
  backToReviewed: (id: string) =>
    http.post(`/api/v1/emails/${id}/back-to-reviewed`).then(r => r.data),
  getDetectedProducts: (id: string) =>
    http.get<{ intent: string[]; products: Array<{ sku: string; title: string | null; score: number | null; status: string | null; image_url: string | null }> }>(`/api/v1/emails/${id}/detected-products`).then(r => r.data),
